...
const AmbilGambar = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== "granted") {
        alert("Izin akses lokasi diperlukan!");
        return;
      }
  ...
  const handleImageUpload = useCallback(async () => {
    ...
    try {
      ...
      if (location) {
        ...
        formData.append("longtitude", String(location.coords.longitude));
      }
      ...
      if (response.data.status) {
        navigation.replace("HasilDeteksiSakit-screen", { id: response.data.data.id });
      } else {
        alert(response.data.message || "Gagal mengupload gambar");
      }
    } catch (error) {
      console.error("Upload error:", error);
      alert(`Gagal mengupload gambar: ${error.response?.data?.err || error.message}`);
    } finally {
      setIsUploading(false);
    }
  }, [photo, location, navigation]);
    ...
    >
      <View className="w-[95%] mt-10">
        ...
        <View className="h-[80%] mt-4 items-center">
          {photo ? (
            <View style={[styles.shadow]} className="w-full aspect-square rounded-lg">
              <Image source={{ uri: photo }} className="w-full aspect-square rounded-lg" />
            </View>
          ) : (
            <View className="w-full aspect-square rounded-lg bg-white justify-center items-center" style={[styles.shadow]}>
              ...
              <MaterialCommunityIcons name={"arrow-down"} size={80} color="#166534" />
            </View>
          )}
            ...
            <View className="w-[48%]">
              ...
              >
                <View className="flex-row h-full items-center justify-center">
                  <MaterialCommunityIcons name={"camera-outline"} size={30} color="#166534" />
                  <Text className="ml-1 text-xl font-semibold leading-7 tracking-widest text-[#40513B]">Kamera</Text>
                </View>
              </TouchableHighlight>
            </View>
            <View className="w-[48%]">
              ...
              >
                <View className="flex-row h-full items-center justify-center">
                  <MaterialCommunityIcons name={"image-outline"} size={30} color="#166534" />
                  <Text className="ml-1 text-xl font-semibold leading-7 tracking-widest text-[#40513B] text-center">Galeri</Text>
                </View>
              </TouchableHighlight>
            </View>
          </View>
          {photo && (
            <TouchableHighlight
              className={`w-full h-[68px] ${isUploading ? 'bg-gray-400' : 'bg-[#166534]'} justify-center rounded-lg mt-5`}
            ...
            >
              <View className="flex-row h-full items-center justify-center">
                {isUploading ? (
                  ...
                  <>
                    <MaterialCommunityIcons name={"upload-outline"} size={30} color="#fff" />
